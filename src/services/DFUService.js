...
// REPORT_USER_CONFIG_SIZE(29) - TRANSPORT_HEADER(4) = 25. Firmware's
// handle_dfu_data just appends bytes into the sync buffer, so chunk size only
// affects host-side framing. NCS dfu.py uses 9 historically (it predates the
// canonical 29-byte report); we use the full 25 here, giving ~2.8x throughput.
const MAX_CHUNK_DATA_BYTES = 25;

import { ConfigStatus, buildEventId } from './WebHIDService.js';
...
//
async function dfuExchange(svc, modId, optId, status, dataBytes = null, opts = {}) {
  ...
  return await svc.exchangeFeatureReport(0, eventId, status, data, { ...DFU_OPTS, ...opts });
}

export async function fwinfo(svc, modId, optId) {
  const r = await dfuExchange(svc, modId, optId, ConfigStatus.FETCH);
...
export async function syncFetch(svc, modId, optId, opts = {}) {
  const r = await dfuExchange(svc, modId, optId, ConfigStatus.FETCH, null, opts);
...
export async function startSet(svc, modId, optId, length, csum, offset) {
  const buf = new ArrayBuffer(12);
  const v = new DataView(buf);
  v.setUint32(0, length, true);
  v.setUint32(4, csum, true);
  v.setUint32(8, offset, true);
  const r = await dfuExchange(svc, modId, optId, ConfigStatus.SET, new Uint8Array(buf));
  return r != null;
}

export async function sendDataChunk(svc, modId, optId, chunkBytes, opts = {}) {
  const r = await dfuExchange(svc, modId, optId, ConfigStatus.SET, chunkBytes, opts);
  return r != null;
}

export async function rebootRequest(svc, modId, optId) {
  const r = await dfuExchange(svc, modId, optId, ConfigStatus.FETCH);
  return r != null;
}

// Wait while STORING or CLEANING. Returns the last sync info, or null on failure / abort.
async function waitUntilNotBusy(svc, modId, optSync, { signal, onPhase } = {}) {
  ...
  while (true) {
    ...
    const info = await syncFetch(svc, modId, optSync, { verbose: false });
    if (!info) return null;
...
export async function dfuTransfer(svc, dfuMod, imageBytes, { onProgress, onPhase, signal } = {}) {
  ...
  onPhase?.('syncing');
  let info = await waitUntilNotBusy(svc, modId, optSync, { signal, onPhase });
  ...
  if (!(await startSet(svc, modId, optStart, total, csum, offset))) {
    return { ok: false, error: 'start rejected' };
  }

  // After start, the device may need a CLEANING pass for the destination slot.
  info = await waitUntilNotBusy(svc, modId, optSync, { signal, onPhase });
  ...
  while (offset < total) {
    ...
    const chunkEnd = Math.min(offset + MAX_CHUNK_DATA_BYTES, total, nextCheckpoint);
    const chunk = imageBytes.subarray(offset, chunkEnd);

    if (!(await sendDataChunk(svc, modId, optData, chunk, chunkOpts))) {
      return { ok: false, error: `data chunk rejected at offset ${offset}` };
    }
    offset = chunkEnd;
    onProgress?.(offset, total);

    if (offset >= nextCheckpoint || offset >= total) {
      const cp = await waitUntilNotBusy(svc, modId, optSync, { signal, onPhase });
  ...
  const finalInfo = await waitUntilNotBusy(svc, modId, optSync, { signal, onPhase });
  if (!finalInfo) {
    return { ok: false, error: signal?.aborted ? 'aborted' : 'final sync failed' };
  }

  onPhase?.('rebooting');
  await rebootRequest(svc, modId, optReboot);

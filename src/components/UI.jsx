...
export const ToggleSwitch = ({ checked, onChange }) => (
  ...
  >
    <div
      className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-black shadow-md transform transition-transform duration-200 ${
    ...
    <div className={`flex ${layout === 'horizontal' ? 'gap-8' : 'gap-4'}`}>
      {options.map((opt) => (
        <label key={opt} className="flex flex-col items-center gap-2 cursor-pointer group">
          ...
          <span className={`text-xs ${String(selected) === String(opt) ? 'text-white' : 'text-gray-500'} font-medium`}>
            {opt}
          </span>
        </label>
      ))}
